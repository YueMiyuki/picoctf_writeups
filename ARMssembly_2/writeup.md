...
The algorithm walks through 23 data bytes, 8 bits each (184 bits total). For each data bit, it writes into the password, but skips bit 7 of each pasword byte. That gives 7 usable bits per password byte, and 184 / 7 = 26.28, so 27 password bytes, with the last one partly filled.
